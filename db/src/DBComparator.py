...
def _write_differences_to_csv(differences, object_type, identifier_keys, output_dir):
    ...
    additional_columns = [
        key for key in identifier_keys if key not in ["schema", "nom", "name"]
    ]
    ...
    header_columns = [
        column_translations.get(col, col) for col in base_columns + additional_columns
    ]
    ...
    try:
        with open(csv_file, mode="w", newline="", encoding="utf-8") as csvfile:
            ...
            for diff in differences:
                ...
                writer.writerow(row)
        logger.info(f"Successfully wrote differences to {csv_file}")
    except Exception as e:
        logger.info(f"Failed to write CSV file {csv_file}. Reason: {e}")
...
class DBObjectComparator(Comparator):
    ...
    def _compare_objects_generic(
        self, objects1, objects2, identifier_keys, object_type
    ):
        ...
        if differences:
            _write_differences_to_csv(
                differences, object_type, identifier_keys, self.output_dir
            )
